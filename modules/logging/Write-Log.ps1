<#
    Logging helpers for OpenIDSync scripts.
    Provides RFC 5424 syslog formatting with optional file and UDP targets,
    plus console output with color highlighting. All state is stored in the
    script scope ($script:Logger).
#>
...
function New-SyslogLine {
    ...
    $fac = 16
    if ($script:Logger -and -not [object]::ReferenceEquals($script:Logger.Facility, $null)) {
        $fac = [int]$script:Logger.Facility
    }
    $pri = ($fac * 8) + $sev
...
function Initialize-Logger {
    ...
    if ($script:Logger.NetEnabled -and $SyslogServer) {
        try {
            ...
            $addr = $null
            try {
                $resolved = Resolve-DnsName -Name $SyslogServer -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty IPAddress
                if ($resolved) { $addr = [System.Net.IPAddress]::Parse($resolved) }
            } catch {}
            if (-not $addr) {
                $addr = ([System.Net.Dns]::GetHostAddresses($SyslogServer) | Select-Object -First 1)
            }
            if ($addr) {
...
function Write-Syslog {
    ...
    if ($script:Logger -and $script:Logger.FileEnabled -and $script:Logger.FilePath) {
        try {
            [System.IO.File]::AppendAllText($script:Logger.FilePath, $line + [Environment]::NewLine, [System.Text.Encoding]::UTF8)
        } catch {}
    }
    ...
    try {
        Write-Host $human -ForegroundColor $color
    } catch {
        Write-Host $human
    }
}
...
function Write-Log {
    ...
    try {
        ...
        if ($script:Logger -and $script:Logger.FileEnabled -and $script:Logger.FilePath) {
            try { $line | Out-File -FilePath $script:Logger.FilePath -Encoding UTF8 -Append } catch {}
        }
        Write-Host $line
    }
}
