...
.DESCRIPTION
    ...
    named 'DomainPromotionConfig' to deploy a new AD DS forest using Microsoft ActiveDirectoryDsc.
    ...
    from the provided JSON. It will:
      ...
      - Compile and apply DSC configuration that calls ADDSInstall to create a new forest
...
$AdministratorUsername = if ($dp.AdministratorUsername) { $dp.AdministratorUsername } else { 'Administrator' }
...
$SafeModeAdministratorPassword = Read-Host -AsSecureString 'Safe Mode Administrator Password'
if (-not $SafeModeAdministratorPassword) { throw 'DSRM password is required.' }
...
$requiredModules = @(
    @{ Name = 'PSDscResources'; MinimumVersion = '2.12.0.0' },
...
Import-Module PSDscResources -ErrorAction Stop
...
Configuration DomainPromotionConfig {
    ...
    Node $NodeName {
        # Use ADDSInstall to create a new forest
        ADDSForest CreateNewForest {
...
DomainPromotionConfig -NodeName $nodeName -DomainName $DomainName -DomainNetbiosName $NetBIOSName -SafeModeAdministratorPassword $SafeModeAdministratorPassword -InstallDNS:$InstallDNS -CreateDNSDelegation:$DNSDelegation -DatabasePath $DatabasePath -LogPath $LogPath -SysvolPath $SYSVOLPath -OutputPath $staging
